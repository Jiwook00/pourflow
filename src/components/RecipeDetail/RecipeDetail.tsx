...
export default function RecipeDetail() {
  ...
  if (!recipe) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-warm-400">레시피를 찾을 수 없습니다.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 pb-28">
      <header className="flex items-center gap-2 py-4 mb-4">
        <button onClick={() => navigate('/')} className="text-warm-400 text-xl px-1">
          ←
        </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-warm-200">
        <div className="max-w-md mx-auto">
          <button
            onClick={handleStart}
            className="w-full bg-brew-500 hover:bg-brew-600 active:bg-brew-700 text-white py-4 rounded-2xl text-lg font-semibold transition-colors"
          >
            ▷ 시작하기
          </button>
        </div>
